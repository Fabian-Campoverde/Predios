...
class ViewDashboard(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        #     except Profile.DoesNotExist:
        #         # Si no se encuentra ningún perfil, puedes manejar la excepción aquí
        #         context['error_message'] = "El perfil del usuario no existe"
...
def list_historial(request):
    ...
    user_profile = Profile.objects.get(user=request.user)
    historial = user_profile.deuda_set.filter(estado='cancelado') | user_profile.deuda_set.filter(estado='vencido')
    context = {
        'user_profile': user_profile,
        'historial': historial,
