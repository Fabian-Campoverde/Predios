...
class DeleteDeuda(DeleteView):
    model = Deuda
    template_name = 'delete.html'
    success_url = reverse_lazy('deudas')  # Redirección exitosa después de la eliminación
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context['title'] = 'Eliminación de una Deuda'
        context['list_url'] = reverse_lazy('deudas')
        context['entity'] = 'Deudas'
        context['action'] = 'del'
        return context
    ...
    def send_email(self, deuda):
        try:
            ...
            content= render_to_string("email.html", {"user": deuda.contribuyente.user,"contrib": deuda.contribuyente ,
                                                     "img":"https://elnuevoempresario.com/wp-content/uploads/2023/02/deuda.jpg", 
                                                     "title":"su deuda eliminada",
                                                     "message": "Su deuda registrada por un monto de S/ "+ str(deuda.monto_deuda)+ " ha sido eliminada del sistema"})
